...
import type { TImageInfo, TUpdateCallback } from '../types'
...
export function rotate(imgId: number, rotation: number, direction: TDir | string, callback: TUpdateCallback<TImageInfo>) {
...
function request(imgId: number, angle: number, callback: TUpdateCallback<TImageInfo>) {
  const url = `${URL}${imgId}/`
  const body = JSON.stringify({ rotation: angle })
  putData<TImageInfo>(url, body, true).then(({ data, error }) => {
